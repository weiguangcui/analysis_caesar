...
def sche_func(m_HI,phi_s,m_s,alpha):
    phi = np.log(10) * 10**phi_s * (m_HI/m_s)**(alpha+1) * np.exp(-m_HI/m_s)
    return np.log10(phi)

def sche_func_mags(mag,phi_s,m_s,alpha):
    phi = 0.4*np.log(10) * 10**phi_s * 10**(-0.4*(mag-m_s)*(alpha+1)) * np.exp(-10**(-0.4*(mag-m_s)))
    return np.log10(phi)
